...
interface AuditLogEntry {
  id: string; timestamp: string; adminId: string; adminUsername: string;
  action: 'UPDATE' | 'DELETE'; recordId: string; details: string;
}
...
const generateChangeLogDetails = (original: ServiceRecord, updated: ServiceRecord): string => {
  ...
  if (original.locationName !== updated.locationName) changes.push(`Nome do Local de "${original.locationName}" para "${updated.locationName}"`);
  if (original.serviceType !== updated.serviceType) changes.push(`Tipo de Serviço de "${original.serviceType}" para "${updated.serviceType}"`);
  if (original.locationArea !== updated.locationArea) changes.push(`Metragem de "${original.locationArea || 0}" para "${updated.locationArea || 0}"`);
  const beforeAdded = updated.beforePhotos.filter(p => !original.beforePhotos.includes(p)).length;
  const beforeRemoved = original.beforePhotos.filter(p => !updated.beforePhotos.includes(p)).length;
  if (beforeAdded || beforeRemoved) {
    let t = 'Fotos "Antes": ';
    if (beforeAdded) t += `adicionou ${beforeAdded}`;
    if (beforeAdded && beforeRemoved) t += ', ';
    if (beforeRemoved) t += `removeu ${beforeRemoved}`;
    changes.push(t);
  }
  const afterAdded = updated.afterPhotos.filter(p => !original.afterPhotos.includes(p)).length;
  const afterRemoved = original.afterPhotos.filter(p => !updated.afterPhotos.includes(p)).length;
  if (afterAdded || afterRemoved) {
    let t = 'Fotos "Depois": ';
    if (afterAdded) t += `adicionou ${afterAdded}`;
    if (afterAdded && afterRemoved) t += ', ';
    if (afterRemoved) t += `removeu ${afterRemoved}`;
    changes.push(t);
  }
  return changes.length ? changes.join('; ') : 'Nenhuma alteração de dados foi feita.';
};
...
// --- Componentes UI ---
const Header: React.FC<{ view: View; currentUser: User | null; onBack?: () => void; onLogout: () => void; }>
= ({ view, currentUser, onBack, onLogout }) => {
...
= ({ onCapture, onCancel, onFinish, photoCount }) => {
  ...
  useEffect(() => {
    ...
    navigator.mediaDevices.getUserMedia({ video: { facingMode: { ideal: "environment" } } })
      .then(mediaStream => {
        if (isMounted) { setStream(mediaStream); if (videoRef.current) videoRef.current.srcObject = mediaStream; }
      })
      .catch(err => {
        console.error("Camera access failed:", err);
        let message = "Acesso à câmera negado.";
        if (err instanceof DOMException) {
          if (err.name === "NotFoundError" || err.name === "DevicesNotFoundError") message = "Nenhuma câmera encontrada. Conecte uma câmera e tente novamente.";
          else if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") message = "A permissão para acessar a câmera foi negada. Habilite nas configurações do seu navegador.";
          else if (err.name === "OverconstrainedError" || err.name === "ConstraintNotSatisfiedError") message = "A câmera traseira não foi encontrada. Verifique se outra aplicação não a está utilizando.";
        }
        alert(message); onCancel();
      });
    return () => { isMounted = false; stream?.getTracks().forEach(t => t.stop()); };
  }, [onCancel]);
...
const AdminDashboard: React.FC<{ onNavigate: (view: View) => void; onBackup: () => void; onRestore: () => void; }>
...
const OperatorGroupSelect: React.FC<{ user: User; onSelectGroup: (group: string) => void; }>
= ({ user, onSelectGroup }) => {
...
const OperatorServiceSelect: React.FC<{
  location: LocationRecord; services: ServiceDefinition[]; user: User; onSelectService: (service: ServiceDefinition) => void;
...
const OperatorLocationSelect: React.FC<{
  locations: LocationRecord[]; contractGroup: string; onSelectLocation: (loc: LocationRecord, gpsUsed: boolean) => void;
  ...
  useEffect(() => {
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        setGpsLocation(currentCoords); setError(null);
  ...
  const handleConfirmNearby = () => { if (nearbyLocation) onSelectLocation(nearbyLocation, true); };
  const handleConfirmNewManual = () => {
    ...
    } else alert('Por favor, digite o nome do novo local.');
  };
  const handleSelectFromList = (loc: LocationRecord) => onSelectLocation(loc, false);
  const filteredLocations = contractLocations.filter(loc => loc.name.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="card">
      ...
      <div className="card-inset">
        ...
        <input type="search" placeholder="Digite para buscar um local..." value={searchQuery}
               onChange={e => setSearchQuery(e.target.value)} style={{marginBottom: '1rem'}} />
      ...
      <div className="card-inset">
        ...
        <input type="text" placeholder="Digite o nome do NOVO local" value={manualLocationName}
...
const PhotoStep: React.FC<{ phase: 'BEFORE' | 'AFTER'; onComplete: (photos: string[]) => void; onCancel: () => void; }>
...
= ({ recordData, onSave, onCancel }) => (
  <div className="card">
    ...
    <div className="detail-section" style={{textAlign: 'left'}}>
      ...
      {recordData.locationArea ? (
        <p><strong>Metragem:</strong> {recordData.locationArea} {recordData.serviceUnit}</p>
      ) : <p><strong>Metragem:</strong> Não informada (novo local)</p>}
...
const HistoryView: React.FC<{
  records: ServiceRecord[]; onSelect: (record: ServiceRecord) => void; isAdmin: boolean;
  onEdit?: (record: ServiceRecord) => void; onDelete?: (recordId: string) => void;
...
const ReportsView: React.FC<{ records: ServiceRecord[]; services: ServiceDefinition[]; }>
= ({ records, services }) => {
  ...
  const handleServiceFilterChange = (service: string, isChecked: boolean) =>
    setSelectedServices(prev => isChecked ? [...prev, service] : prev.filter(s => s !== service));
      ...
      <div className="card report-filters">
        ...
        <div className="form-group">
          ...
          <select id="group-filter" value={selectedContractGroup} onChange={e => setSelectedContractGroup(e.target.value)}>
            ...
            {allContractGroups.map(group => (<option key={group} value={group}>{group}</option>))}
          </select>
        </div>
          ...
          <div className="checkbox-group">
            {allServiceNames.map(service => (
              <div key={service} className="checkbox-item">
                <input type="checkbox" id={`service-${service}`} checked={selectedServices.includes(service)}
                       onChange={e => handleServiceFilterChange(service, e.target.checked)} />
...
const ManageLocationsView: React.FC<{
  locations: LocationRecord[]; setLocations: React.Dispatch<React.SetStateAction<LocationRecord[]>>;
  ...
  const resetForm = () => { setName(''); setArea(''); setCoords(null); setSelectedServiceIds(new Set()); setEditingId(null); };
  const handleAddNewGroup = () => {
    const newGroup = prompt('Digite o nome do novo Contrato/Cidade:');
    if (newGroup) { setSelectedGroup(newGroup); resetForm(); }
  };
  const handleGetCoordinates = () => {
    setIsFetchingCoords(true);
    navigator.geolocation.getCurrentPosition(
      (position) => { setCoords({ latitude: position.coords.latitude, longitude: position.coords.longitude }); setIsFetchingCoords(false); },
      (error) => { alert(`Erro ao obter GPS: ${error.message}`); setIsFetchingCoords(false); },
      { enableHighAccuracy: true }
    );
  };
  ...
  const handleServiceCheckbox = (serviceId: string, checked: boolean) => {
    setSelectedServiceIds(prev => {
      ...
      if (checked) n.add(serviceId); else n.delete(serviceId);
      return n;
    });
  };

  const handleSave = async () => {
    if (!selectedGroup) { alert('Selecione um Contrato/Cidade.'); return; }
    if (!name) { alert('O nome do local é obrigatório.'); return; }
    if (selectedServiceIds.size > 0 && (!area || isNaN(parseFloat(area)))) {
      alert('A metragem é obrigatória quando um serviço é selecionado.'); return;
    }
    const payload = {
        ...
        <>
              ...
              <div className="checkbox-group">
                {services.sort((a,b) => a.name.localeCompare(b.name)).map(service => (
                  <div key={service.id} className="checkbox-item">
                    <input type="checkbox" id={`service-loc-${service.id}`} checked={selectedServiceIds.has(service.id)}
          ...
          <ul className="location-list">
            {filteredLocations.sort((a,b) => a.name.localeCompare(b.name)).map(loc => {
              const serviceNames = (loc.serviceIds || []).map(id => services.find(s => s.id === id)?.name).filter(Boolean) as string[];
              return (
                <li key={loc.id} className="card list-item">
                  <div className="list-item-info">
                    ...
                    <p className="location-services-list"><strong>Serviços:</strong> {serviceNames.length > 0 ? serviceNames.join(', ') : 'Nenhum atribuído'}</p>
...
const ManageUsersView: React.FC<{
  users: User[]; onUsersUpdate: () => Promise<void>;
  services: ServiceDefinition[]; locations: LocationRecord[];
}> = ({ users, onUsersUpdate, services, locations }) => {
  const [username, setUsername] = useState(''); const [email, setEmail] = useState('');
  const [password, setPassword] = useState(''); const [role, setRole] = useState<Role>('OPERATOR');
  ...
  const resetForm = () => { setUsername(''); setPassword(''); setEmail(''); setRole('OPERATOR'); setAssignments([]); setEditingId(null); };
  const handleAddAssignment = () => {
    if (!newAssignmentGroup) { alert('Por favor, selecione um Contrato/Cidade.'); return; }
    if (newAssignmentServices.size === 0) { alert('Por favor, selecione pelo menos um serviço.'); return; }
    if (assignments.some(a => a.contractGroup === newAssignmentGroup)) {
      alert('Este contrato já foi atribuído. Remova o antigo para adicionar um novo com serviços diferentes.'); return;
    }
    setAssignments(prev => [...prev, { contractGroup: newAssignmentGroup, serviceNames: Array.from(newAssignmentServices) }]
      .sort((a,b) => a.contractGroup.localeCompare(b.contractGroup)));
    setNewAssignmentGroup(''); setNewAssignmentServices(new Set());
  };
  const handleRemoveAssignment = (groupToRemove: string) => setAssignments(prev => prev.filter(a => a.contractGroup !== groupToRemove));
  const handleServiceCheckbox = (serviceName: string, checked: boolean) => {
    setNewAssignmentServices(prev => { const s = new Set(prev); if (checked) s.add(serviceName); else s.delete(serviceName); return s; });
  };

  const handleSave = async () => {
    if (!username || !email) { alert('Nome e e-mail são obrigatórios.'); return; }
    if (!editingId && !password) { alert('A senha é obrigatória para novos usuários.'); return; }
    setIsLoading(true);
    const payload: any = { name: username, email, role };
    if (password) payload.password = password;
    if (role === 'OPERATOR' || role === 'FISCAL') payload.assignments = assignments;
    try {
      if (editingId) {
  ...
  const handleEdit = (user: User) => {
    setEditingId(user.id); setUsername(user.username); setEmail(user.email || ''); setPassword('');
    setRole(user.role); setAssignments(user.assignments || []);
  };

  const handleDelete = async (id: string) => {
    if(window.confirm('Excluir este usuário? Esta ação não pode ser desfeita.')) {
      setIsLoading(true);
      try { await apiFetch(`/api/users/${id}`, { method: 'DELETE' }); await onUsersUpdate(); }
      catch (e) { alert('Falha ao excluir usuário.'); console.error(e); }
      finally { setIsLoading(false); }
    }
  };
    ...
    <div>
      <div className="form-container card">
        ...
        <button className="button admin-button" onClick={handleSave} disabled={isLoading}>
          {isLoading ? 'Salvando...' : (editingId ? 'Salvar Alterações' : 'Adicionar')}
        </button>
...
const ManageGoalsView: React.FC<{
  ...
  records: ServiceRecord[]; locations: LocationRecord[];
  ...
  const resetForm = () => { setContractGroup(''); setMonth(new Date().toISOString().substring(0, 7)); setTargetArea(''); setEditingId(null); };

  const handleSave = () => {
    if (!contractGroup || !month || !targetArea || isNaN(parseFloat(targetArea))) { alert('Preencha todos os campos corretamente.'); return; }
    const newGoal: Goal = { id: editingId || new Date().toISOString(), contractGroup, month, targetArea: parseFloat(targetArea) };
    if (editingId) setGoals(prev => prev.map(g => g.id === editingId ? newGoal : g));
    else setGoals(prev => [newGoal, ...prev]);
    resetForm();
  };
  const handleEdit = (goal: Goal) => { setEditingId(goal.id); setContractGroup(goal.contractGroup); setMonth(goal.month); setTargetArea(String(goal.targetArea)); };
  const handleDelete = (id: string) => { if (window.confirm('Excluir esta meta?')) setGoals(prev => prev.filter(g => g.id !== id)); };

  return (
    <div>
      <div className="form-container card">
        <h3>{editingId ? 'Editando Meta' : 'Adicionar Nova Meta'} (Local)</h3>
        <input list="goal-contract-groups" placeholder="Digite ou selecione um Contrato/Cidade" value={contractGroup} onChange={e => setContractGroup(e.target.value)} />
        <datalist id="goal-contract-groups">{allGroups.map(g => <option key={g} value={g} />)}</datalist>
...
const ServiceInProgressView: React.FC<{ service: Partial<ServiceRecord>; onFinish: () => void; }>
= ({ service, onFinish }) => (
  <div className="card">
...
const AdminEditRecordView: React.FC<{
  record: ServiceRecord; onSave: (updatedRecord: ServiceRecord) => void; onCancel: () => void; services: ServiceDefinition[];
}> = ({ record, onSave, onCancel }) => {
  const [formData, setFormData] = useState<ServiceRecord>(record);
  const handleChange = (field: keyof ServiceRecord, value: any) => setFormData(prev => ({ ...prev, [field]: value }));

  const handleSave = async () => {
    try {
      const updated = await apiFetch(`/api/records/${formData.id}`, { method: 'PUT', body: JSON.stringify(formData) });
      onSave(updated);
      alert("Registro atualizado com sucesso!");
    } catch (e) {
      alert("Erro ao atualizar registro."); console.error(e);
    }
  };
  ...
  const handlePhotoUpload = async (phase: 'BEFORE' | 'AFTER', files: FileList | null) => {
    ...
    try {
      const updated = await apiFetch(`/api/records/${formData.id}/photos`, { method: "POST", body: formDataUpload });
      setFormData(updated);
    } catch (err) {
  ...
  const handlePhotoRemove = async (phase: 'BEFORE' | 'AFTER', photoUrl: string) => {
    try {
      const newBefore = phase === "BEFORE" ? formData.beforePhotos.filter(p => p !== photoUrl) : formData.beforePhotos;
      const newAfter = phase === "AFTER" ? formData.afterPhotos.filter(p => p !== photoUrl) : formData.afterPhotos;
      const updated = await apiFetch(`/api/records/${formData.id}`, {
        method: "PUT",
        body: JSON.stringify({ ...formData, beforePhotos: newBefore, afterPhotos: newAfter })
      });
      setFormData(updated);
    } catch (err) {
  ...
  return (
    <div className="card edit-form-container">
      <div className="form-group">
        ...
        <input type="text" value={formData.locationName} onChange={e => handleChange("locationName", e.target.value)} />
      </div>
      <div className="form-group">
        <label>Tipo de Serviço</label>
        <input type="text" value={formData.serviceType} onChange={e => handleChange("serviceType", e.target.value)} />
      </div>
      <div className="form-group">
        <label>Medição ({formData.serviceUnit})</label>
        <input type="number" value={formData.locationArea || ''} onChange={e => handleChange("locationArea", parseFloat(e.target.value))} />
      </div>
      <div className="form-group">
        <label>Unidade</label>
        <select value={formData.serviceUnit} onChange={e => handleChange("serviceUnit", e.target.value as 'm²' | 'm linear')}>
      ...
      <div className="form-group">
        ...
        <input type="text" value={formData.contractGroup} onChange={e => handleChange("contractGroup", e.target.value)} />
      </div>
      <div className="form-group">
        <label>Início</label>
        <input type="datetime-local" value={formData.startTime ? formData.startTime.slice(0,16) : ""} onChange={e => handleChange("startTime", e.target.value)} />
      </div>
      <div className="form-group">
        <label>Fim</label>
        <input type="datetime-local" value={formData.endTime ? formData.endTime.slice(0,16) : ""} onChange={e => handleChange("endTime", e.target.value)} />
      </div>
        ...
        <div className="edit-photo-gallery">
          {formData.beforePhotos.map((p, i) => (
            <div key={i} className="edit-photo-item">
              ...
              <button className="button button-sm button-danger" onClick={() => handlePhotoRemove("BEFORE", p)}>❌</button>
            </div>
          ))}
        </div>
        <input type="file" accept="image/*" multiple onChange={e => handlePhotoUpload("BEFORE", e.target.files)} />
      </div>
        ...
        <div className="edit-photo-gallery">
          {formData.afterPhotos.map((p, i) => (
            <div key={i} className="edit-photo-item">
              ...
              <button className="button button-sm button-danger" onClick={() => handlePhotoRemove("AFTER", p)}>❌</button>
            </div>
          ))}
        </div>
        <input type="file" accept="image/*" multiple onChange={e => handlePhotoUpload("AFTER", e.target.files)} />
      </div>
...
const AuditLogView: React.FC<{ log: AuditLogEntry[] }> = ({ log }) => {
  const handleExportPdf = () => {
    ...
    let y = 35; const pageMargin = 14; const pageWidth = doc.internal.pageSize.getWidth() - (pageMargin * 2);
    log.forEach(entry => {
      if (y > 270) { doc.addPage(); y = 20; }
      doc.setFontSize(12); doc.setFont('Helvetica', 'bold');
      doc.text(`Data: ${formatDateTime(entry.timestamp)}`, pageMargin, y); y += 7;
      ...
      y += 5; doc.setDrawColor(200); doc.line(pageMargin, y, pageWidth + pageMargin, y); y += 10;
    });
  ...
  return (
    <div>
      <div className="audit-log-header">
        ...
        <button className="button admin-button" onClick={handleExportPdf} disabled={log.length === 0}>Exportar para PDF</button>
      </div>
      {log.length === 0 ? (<p>Nenhuma alteração administrativa foi registrada ainda.</p>) : (
...
const ManageServicesView: React.FC<{
  services: ServiceDefinition[]; setServices: React.Dispatch<React.SetStateAction<ServiceDefinition[]>>;
}> = ({ services, setServices }) => {
  const [name, setName] = useState(''); const [unit, setUnit] = useState<'m²' | 'm linear'>('m²');
  const [editingId, setEditingId] = useState<string | null>(null);
  const resetForm = () => { setName(''); setUnit('m²'); setEditingId(null); };
  const handleSave = () => {
    if (!name.trim()) { alert('O nome do serviço é obrigatório.'); return; }
    const newService: ServiceDefinition = { id: editingId || `service-${new Date().getTime()}`, name, unit };
    if (editingId) setServices(prev => prev.map(s => s.id === editingId ? newService : s));
    else setServices(prev => [newService, ...prev]);
    resetForm();
  };
  const handleEdit = (service: ServiceDefinition) => { setEditingId(service.id); setName(service.name); setUnit(service.unit); };
...
const App = () => {
  ...
  const navigate = (newView: View, replace = false) => { if (!replace) setHistory(h => [...h, view]); setView(newView); };
  const handleBack = () => {
    const lastView = history.pop();
    if (lastView) { setHistory([...history]); setView(lastView); }
    else if (currentUser) { redirectUser(currentUser); }
  };
  ...
  const handleLogout = () => {
    setCurrentUser(null); setApiToken(null); setHistory([]);
    setSelectedContractGroup(null); setSelectedLocation(null); setCurrentService({});
    setLocations([]); setRecords([]); setUsers([]);
    navigate('LOGIN', true);
  };
    ...
    try {
      if (currentUser.role === 'ADMIN') {
        ...
        setLocations(locs.map((l: any) => ({
          id: String(l.id), contractGroup: l.city, name: l.name, area: l.area || 0,
          coords: (l.lat!=null && l.lng!=null) ? { latitude: l.lat, longitude: l.lng } : undefined,
          serviceIds: l.service_ids || []
        })));
        setRecords(recs.map((r: any) => ({
          ...r, id: String(r.id), contractGroup: r.location_city, operatorId: String(r.operator_id),
  ...
  useEffect(() => {
    const restoreSession = async () => {
      // Evita depender de variável interna do módulo ./api
      if (localStorage.getItem('crbApiToken')) {
  ...
  useEffect(() => { if (currentUser) fetchData(); /* eslint-disable-next-line */ }, [currentUser]);

  const resetService = () => { setCurrentService({}); setSelectedContractGroup(null); setSelectedLocation(null); if (currentUser) redirectUser(currentUser); };

  const handleLogin = (user: User) => { setCurrentUser(user); redirectUser(user); };
  const handleBackup = () => alert("O backup agora deve ser realizado diretamente no servidor/banco de dados.");
  const handleRestore = () => alert("A restauração de dados agora deve ser realizada diretamente no servidor/banco de dados.");

  const handleGroupSelect = (group: string) => { setSelectedContractGroup(group); navigate('OPERATOR_LOCATION_SELECT'); };
  const handleLocationSelect = (location: LocationRecord, gpsUsed: boolean) => {
    setSelectedLocation({ ...location, _gpsUsed: gpsUsed } as any);
    const servicesForLocation = location.serviceIds ? services.filter(s => location.serviceIds!.includes(s.id)) : [];
    if (servicesForLocation.length === 1) handleServiceSelect(servicesForLocation[0]);
    else navigate('OPERATOR_SERVICE_SELECT');
  };
  ...
  const handleBeforePhotos = async (photos: string[]) => {
    ...
    try {
      ...
      console.error(err); setIsLoading(null); alert("Falha ao salvar registro local.");
    }
  };

  // Salva as fotos "Depois"
  const handleAfterPhotos = async (photos: string[]) => {
    if (!currentService?.id) { alert("Não encontrei o registro em andamento. Recomece o processo, por favor."); return; }
    ...
    try {
      ...
      console.error(err); setIsLoading(null); alert("Falha ao salvar fotos localmente.");
    }
  };

  // Concluir (apenas encerra o fluxo e volta)
  const handleSave = () => {
    alert("Registro salvo e será sincronizado automaticamente quando houver conexão.");
    resetService();
  };
  ...
  const handleEditRecord = (record: ServiceRecord) => { setSelectedRecord(record); navigate('ADMIN_EDIT_RECORD'); };
  ...
  const handleDeleteRecord = async (recordId: string) => {
    ...
    if (window.confirm(`Tem certeza que deseja excluir o registro do local "${recordToDelete.locationName}"? Esta ação não pode ser desfeita.`)) {
      try {
        ...
        alert("Falha ao excluir o registro."); console.error(e);
      }
    }
  };

  const renderView = () => {
    if (!currentUser && view !== 'LOGIN') return <Loader text="Verificando sessão..." />;
    ...
    switch(currentUser.role) {
      ...
      case 'OPERATOR':
        switch(view) {
          ...
          case 'PHOTO_STEP':
            if(!currentService.id) return <PhotoStep phase="BEFORE" onComplete={handleBeforePhotos} onCancel={resetService} />;
